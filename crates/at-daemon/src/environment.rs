...
fn set_default_env_vars() {
    ...
    env::set_var("DD_SERVICE", env::var("DD_SERVICE").unwrap_or_else(|_| "at-daemon".to_string()));
    env::set_var("DD_ENV", env::var("DD_ENV").unwrap_or_else(|_| "development".to_string()));
    env::set_var("DD_VERSION", env::var("DD_VERSION").unwrap_or_else(|_| "0.1.0".to_string()));
    
    // Agent configuration
    env::set_var("DD_TRACE_AGENT_URL", env::var("DD_TRACE_AGENT_URL").unwrap_or_else(|_| "http://localhost:8126".to_string()));
    env::set_var("DD_TRACE_AGENT_PORT", env::var("DD_TRACE_AGENT_PORT").unwrap_or_else(|_| "8126".to_string()));
    
    // Tracing configuration
    env::set_var("DD_TRACE_ENABLED", env::var("DD_TRACE_ENABLED").unwrap_or_else(|_| "true".to_string()));
    env::set_var("DD_TRACE_SAMPLE_RATE", env::var("DD_TRACE_SAMPLE_RATE").unwrap_or_else(|_| "1.0".to_string()));
    
    // Profiling configuration
    env::set_var("DD_PROFILING_ENABLED", env::var("DD_PROFILING_ENABLED").unwrap_or_else(|_| "true".to_string()));
    
    // Application configuration
    env::set_var("RUST_LOG", env::var("RUST_LOG").unwrap_or_else(|_| "info,at_daemon=debug".to_string()));
}
...
fn log_datadog_config() {
    ...
    info!("  Agent URL: {}", env::var("DD_TRACE_AGENT_URL").unwrap_or_default());
    info!("  Trace Enabled: {}", env::var("DD_TRACE_ENABLED").unwrap_or_default());
    info!("  Sample Rate: {}", env::var("DD_TRACE_SAMPLE_RATE").unwrap_or_default());
    info!("  Profiling Enabled: {}", env::var("DD_PROFILING_ENABLED").unwrap_or_default());
    
    if let Ok(api_key) = env::var("DD_API_KEY") {
        info!("  API Key: {}***", &api_key[..std::cmp::min(8, api_key.len())]);
    } else {
...
pub fn validate_environment() -> Result<()> {
    ...
    if let Ok(sample_rate) = env::var("DD_TRACE_SAMPLE_RATE") {
        let rate: f64 = sample_rate.parse()
            .context("DD_TRACE_SAMPLE_RATE must be a valid number")?;
        if rate < 0.0 || rate > 1.0 {
            return Err(anyhow::anyhow!("DD_TRACE_SAMPLE_RATE must be between 0.0 and 1.0"));
        }
    }
