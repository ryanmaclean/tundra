...
impl TaskOrchestrator {
    ...
    ///
    /// `max_gate_fix_iterations` and `qa_checker` are set to the same
    /// defaults [`TaskOrchestrator::new`] uses; use [`with_max_gate_fix_iterations`](Self::with_max_gate_fix_iterations)
    /// / [`with_qa_checker`](Self::with_qa_checker) on the result to override them.
    ///
