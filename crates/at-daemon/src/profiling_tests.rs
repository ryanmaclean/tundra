...
impl ProfilingTests {
    ...
    pub async fn test_llm_observability() -> Result<()> {
        ...
        crate::metrics::AppMetrics::llm_profile_bootstrap(3, "test-model", "test-provider").await;

        // Test LLM request metrics
        crate::metrics::AppMetrics::llm_request("gpt-4", "openai", 150, 1250.5, true).await;

        // Test LLM agent execution
        crate::metrics::AppMetrics::llm_agent_execution("spec-agent", "code-analysis", 850.2, true)
            .await;
    ...
    pub async fn test_performance_load() -> Result<()> {
        ...
        let tasks = (0..100).map(|i| async move {
            ...
            // Record metrics
            crate::metrics::AppMetrics::task_executed("load_test_task", true, 1.0).await;
    ...
    pub async fn test_error_handling() -> Result<()> {
        ...
        crate::metrics::AppMetrics::error_occurred("test_error", "test_component").await;

        // Test failed LLM request
        crate::metrics::AppMetrics::llm_request("gpt-4", "openai", 0, 5000.0, false).await;
    ...
    pub async fn test_concurrent_operations() -> Result<()> {
        ...
        for i in 0..10 {
            let handle = tokio::spawn(async move {
                ...
                match i % 3 {
                    0 => {
                        crate::metrics::AppMetrics::api_request("GET", "/api/test", 200, 150.0)
                            .await;
                    }
                    1 => {
                        crate::metrics::AppMetrics::frontend_request("/test", 200, 50.0).await;
                    }
                    _ => {
                        crate::metrics::AppMetrics::task_executed("concurrent_task", true, 75.0)
                            .await;
                    }
                }
