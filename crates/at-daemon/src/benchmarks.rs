...
impl PerformanceBenchmarks {
    ...
    pub async fn benchmark_span_creation() -> Result<()> {
        ...
        println!("   Rating: {}", if per_span < 100 { "🟢 Excellent" } else if per_span < 200 { "🟡 Good" } else { "🔴 Needs Optimization" });
    ...
    pub async fn benchmark_metrics_recording() -> Result<()> {
        ...
        println!("   Rating: {}", if per_metric < 50_000 { "🟢 Excellent" } else if per_metric < 100_000 { "🟡 Good" } else { "🔴 Needs Optimization" });
    ...
    pub async fn benchmark_concurrent_operations() -> Result<()> {
        ...
        for task_id in 0..concurrent_tasks {
            let handle = tokio::spawn(async move {
                ...
                for op_id in 0..operations_per_task {
                    let _span = crate::traced_span!("concurrent_benchmark", 
                        task_id = task_id, 
                        operation_id = op_id
                    );
        ...
        println!("   Rating: {}", if throughput > 10_000.0 { "🟢 Excellent" } else if throughput > 5_000.0 { "🟡 Good" } else { "🔴 Needs Optimization" });
    ...
    pub async fn benchmark_memory_usage() -> Result<()> {
        ...
        println!("   Rating: {}", if memory_per_span < 0.5 { "🟢 Excellent" } else if memory_per_span < 1.0 { "🟡 Good" } else { "🔴 Needs Optimization" });
    ...
    pub async fn benchmark_datadog_throughput() -> Result<()> {
        ...
        while start.elapsed() < Duration::from_secs(duration_seconds) {
            ...
            for i in 0..events_per_second {
                crate::profiling::record_event("benchmark_event", &[
        ...
        println!("   Total duration: {:?}", total_duration);
        println!("   Rating: {}", if actual_throughput > 800.0 { "🟢 Excellent" } else if actual_throughput > 500.0 { "🟡 Good" } else { "🔴 Needs Optimization" });
...
mod tests {
    ...
    async fn test_performance_benchmarks() {
        PerformanceBenchmarks::run_all_benchmarks().await.expect("Benchmarks should complete successfully");
    }
}
