...
mod pipeline_tests {
    ...
    async fn run_happy_path_completes_all_phases() {
        ...
        // Pre-load one neutral chunk per non-Complete phase. Each chunk carries
        // the phase-complete sentinel on its own line so `collect_phase_output`
        // stops after exactly one chunk instead of continuing to accumulate
        // (and draining) the next phase's pre-loaded chunk too.
        for _ in 0..EXPECTED_PHASE_COUNT {
            read_tx
                .send(format!("normal agent output\n{PHASE_COMPLETE_SENTINEL}\n").into_bytes())
                .unwrap();
        }
    ...
    async fn run_phase_error_bails_out() {
        ...
        // Pre-load neutral chunks for phases before Coding. Each carries the
        // phase-complete sentinel so `collect_phase_output` stops after one
        // chunk per phase (see `run_happy_path_completes_all_phases`).
        for _ in 0..CODING_INDEX {
            read_tx
                .send(format!("normal output\n{PHASE_COMPLETE_SENTINEL}\n").into_bytes())
                .unwrap();
        }
