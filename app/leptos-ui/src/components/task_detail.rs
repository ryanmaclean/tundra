...
fn branch_name(title: &str) -> String {
    ...
    let short = if trimmed.len() > 40 { &trimmed[..40] } else { &trimmed };
    format!("feat/{}", short.trim_end_matches('-'))
}

fn execution_log(bead: &BeadResponse) -> Vec<(String, String, &'static str)> {
    let mut logs = vec![("Task created".to_string(), bead.timestamp.clone(), "log-info")];
    match bead.status {
        BeadStatus::Planning => {
            logs.push(("Planning phase started".to_string(), bead.timestamp.clone(), "log-info"));
        }
        BeadStatus::InProgress => {
            logs.push(("Planning completed".to_string(), "earlier".to_string(), "log-success"));
            logs.push(("Implementation started".to_string(), bead.timestamp.clone(), "log-info"));
            if !bead.agent_names.is_empty() {
                logs.push((format!("Assigned to {}", bead.agent_names.join(", ")), bead.timestamp.clone(), "log-info"));
            }
        }
        BeadStatus::AiReview => {
            logs.push(("Planning completed".to_string(), "earlier".to_string(), "log-success"));
            logs.push(("Implementation completed".to_string(), "earlier".to_string(), "log-success"));
            logs.push(("AI review started".to_string(), bead.timestamp.clone(), "log-info"));
        }
        BeadStatus::HumanReview => {
            logs.push(("Planning completed".to_string(), "earlier".to_string(), "log-success"));
            logs.push(("Implementation completed".to_string(), "earlier".to_string(), "log-success"));
            logs.push(("AI review passed".to_string(), "earlier".to_string(), "log-success"));
            logs.push(("Awaiting human review".to_string(), bead.timestamp.clone(), "log-warning"));
        }
        BeadStatus::Done => {
            logs.push(("Planning completed".to_string(), "earlier".to_string(), "log-success"));
            logs.push(("Implementation completed".to_string(), "earlier".to_string(), "log-success"));
            logs.push(("Review passed".to_string(), "earlier".to_string(), "log-success"));
            logs.push(("Task completed".to_string(), bead.timestamp.clone(), "log-success"));
        }
        BeadStatus::Failed => {
            logs.push(("Task failed".to_string(), bead.timestamp.clone(), "log-error"));
        }
    }
    logs
}
...
fn demo_subtasks(bead: &BeadResponse) -> Vec<(String, bool)> {
    ...
    base_tasks.into_iter().enumerate().map(|(i, t)| (t, i < completed_count)).collect()
}

/// Generate demo file tree based on bead
fn demo_file_tree(bead: &BeadResponse) -> Vec<(String, &'static str)> {
    let slug = bead.title.to_lowercase().replace(' ', "_").chars().take(20).collect::<String>();
    vec![
...
fn mock_diff_for_file(path: &str, change_type: &str) -> Vec<(String, &'static str)> {
    match change_type {
        ...
        "modified" => {
            ...
            vec![
                (format!("@@ -10,6 +10,12 @@ // {}", mod_name), "diff-context"),
                (" use std::collections::HashMap;".to_string(), "diff-context"),
                (" ".to_string(), "diff-context"),
                ("-fn old_function() -> bool {".to_string(), "diff-remove"),
                ("-    false".to_string(), "diff-remove"),
                ("+fn improved_function() -> Result<bool, Error> {".to_string(), "diff-add"),
...
fn mock_qa_checks(bead: &BeadResponse) -> (bool, Vec<(String, bool, String)>, Vec<String>) {
    ...
    let checks = vec![
        ("Unit Tests".to_string(), !is_qa_fail, if is_qa_fail { "2 tests failed in module::tests".to_string() } else { "All 24 tests passed".to_string() }),
        ("Integration Tests".to_string(), true, "12 integration tests passed".to_string()),
        ("Clippy Lints".to_string(), !is_qa_fail, if is_qa_fail { "3 warnings found".to_string() } else { "No warnings".to_string() }),
        ("Format Check".to_string(), true, "All files formatted".to_string()),
        ("Security Audit".to_string(), true, "No vulnerabilities found".to_string()),
        ("Documentation".to_string(), true, "All public items documented".to_string()),
    ];
...
fn is_stuck(bead: &BeadResponse) -> bool {
    bead.tags.iter().any(|t| t == "Stuck" || t == "Needs Recovery")
}
...
fn TaskDetailInner(
    ...
    let completed_subtasks = subtasks.iter().filter(|(_, done)| *done).count();
    let total_subtasks = subtasks.len();

    // Tab state: 0=Overview, 1=Subtasks, 2=Logs, 3=Files
    ...
    let default_checked: std::collections::HashSet<usize> = subtasks.iter().enumerate()
        ...
        move |_: MouseEvent| {
            ...
            spawn_local(async move {
                match crate::api::run_task_qa(&bid).await {
                    Ok(report) => {
                        let checks: Vec<(String, bool, String)> = report.checks.iter().map(|c| {
                            (c.name.clone(), c.passed, c.message.clone())
                        }).collect();
                        let all_pass = checks.iter().all(|(_, p, _)| *p);
                        set_qa_report.set((all_pass, checks, report.suggestions));
                    }
                    Err(_) => {
                        set_qa_report.update(|r| { r.0 = !r.0; });
                    }
                }
    ...
    let diff_data: Vec<(String, String, Vec<(String, String)>)> = file_tree.iter().map(|(path, ct)| {
        let lines: Vec<(String, String)> = mock_diff_for_file(path, ct).into_iter()
    ...
    let wt_path = format!("/home/dev/auto-tundra-wt/{}", branch.split('/').last().unwrap_or("work"));
                    ...
                    view! {
                        <div class="task-tab-overview">
                            ...
                            <div class="td-tags-row">
                                ...
                                <span class="td-tag-timestamp">{bead.timestamp.clone()}</span>
                            </div>
                            ...
                            // QA Summary (compact)
                            {(bead.status != BeadStatus::Planning).then(|| {
                                view! {
